...
export default function InventoryScreen() {
  ...
  const handleSave = async () => {
    ...
    if (!categoryId && (!categories || categories.length === 0)) {
      Alert.alert("Error", "No categories found. Please create a category first.");
      return;
    }
    ...
    try {
      ...
      console.error(err);
      Alert.alert("Error", err?.data?.message || "Failed to save product. Check if SKU is unique.");
    }
  };
    ...
    <SafeAreaView className="flex-1 bg-slate-50">
      ...
      {isLoading ? (
        ...
        <View className="flex-1 items-center justify-center px-10">
          <Text className="text-red-500 font-bold text-lg text-center">Error loading inventory.</Text>
        </View>
      ) : (
          ...
          {products?.map((product) => (
            ...
            >
              ...
              <View className="flex-1">
                <Text className="text-slate-900 font-black text-xl" numberOfLines={1}>{product.name}</Text>
      ...
      >
        ...
        >
          <View className="p-8">
            ...
            <ScrollView showsVerticalScrollIndicator={false} className="gap-6" contentContainerStyle={{ paddingBottom: 100 }}>
              <View>
                <Text className="text-slate-500 font-bold mb-2 ml-1">Product Name *</Text>
                <TextInput 
              ...
              <View className="flex-row gap-4">
                <View className="flex-1">
                  <Text className="text-slate-500 font-bold mb-2 ml-1">SKU *</Text>
                  <TextInput 
                    placeholder="SKU-123"
                    value={sku}
                    onChangeText={setSku}
              ...
              <View className="flex-row gap-4">
                <View className="flex-1">
                  <Text className="text-slate-500 font-bold mb-2 ml-1">Selling Price *</Text>
                  <TextInput 
                    placeholder="0.00"
                    value={sellingPrice}
                ...
                <View className="flex-1">
                  <Text className="text-slate-500 font-bold mb-2 ml-1">Cost Price *</Text>
                  <TextInput 
                    placeholder="0.00"
                    value={costPrice}
              ...
              <View className="flex-row gap-4">
                <View className="flex-1">
                  <Text className="text-slate-500 font-bold mb-2 ml-1">Stock Quantity *</Text>
                  <TextInput 
                    placeholder="0"
                    value={stockQuantity}
                ...
                <View className="flex-1">
                  <Text className="text-slate-500 font-bold mb-2 ml-1">Brand</Text>
                  <TextInput 
              ...
              <View>
                <Text className="text-slate-500 font-bold mb-2 ml-1">Description</Text>
                <TextInput 
              ...
              <View>
                <Text className="text-slate-500 font-bold mb-2 ml-1">Category *</Text>
                {categories && categories.length > 0 ? (
                  <View className="flex-row flex-wrap gap-2">
                    {categories.map((cat) => (
                      ...
                      >
                        <Text className={`font-bold ${categoryId === cat.id ? 'text-white' : 'text-slate-600'}`}>{cat.name}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                ) : (
                  <Text className="text-red-500 italic">No categories available. Please create one in the backend.</Text>
                )}
              </View>
              ...
              >
                <Text className="text-white font-black text-xl">Save Product</Text>
              </TouchableOpacity>
            </ScrollView>
          </View>
        </KeyboardAvoidingView>
